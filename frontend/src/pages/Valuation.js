...
import { fmtPct, fmtPctFraction, fmtMoney, fmtNum, fmtLarge } from "../utils/format";
...
function PriceVsIntrinsicBar({ currentPrice, intrinsicValue }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      <div style={{ position: "relative", height: 28, background: "var(--bg)", borderRadius: 6, overflow: "hidden", marginBottom: 6 }}>
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${currentPct}%`, background: "var(--accent)", opacity: 0.7 }} />
        <div style={{ position: "absolute", left: `calc(${intrinsicPct}% - 2px)`, top: 0, bottom: 0, width: 3, background: "var(--green)" }} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--text-secondary)" }}>
        <span>💰 Current: {fmtMoney(currentPrice)}</span>
        <span>🎯 Intrinsic: {fmtMoney(intrinsicValue)}</span>
      </div>
    </div>
  );
}
